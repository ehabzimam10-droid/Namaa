...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const investInProject = async (kidName: string, projectId: string, amount: number) => {
    // a. Find the target project in the local projects state and calculate new investment
    ...
    // c. Update the specific kid's local state (subtract amount from saved)
    setKids((prevKids) =>
      prevKids.map((kid) => {
        if (kid.name === kidName) {
          const updatedSaved = Math.max(0, kid.saved - amount);
    ...
    try {
      ...
      console.error('Failed to sync kid investment to Supabase:', err);
    }
  };
